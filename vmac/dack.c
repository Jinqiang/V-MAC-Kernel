...
/**
 * @brief    Starts dack generation Thread (runs on kernel installation)
 */
void dack_start(void)
{
    kthread_run(&dackgen, (void*)0, "DACKGEN");
}

/**
 * @brief      Initialize DACK queues and spinlock.
 */
void dack_init(void)
{
    INIT_LIST_HEAD(&dqueue.list);
    INIT_LIST_HEAD(&dackfree.list);
    spin_lock_init(&dackfreelok);
    dackreqnum = 0;
}
 ...
 *
 * @param      data : N/A not being used
 *
 * @return     None: runs forever
 * @code{.unparsed}
 * 
 * while forever
 *  sleep 100-1000 usecs
 *  start going over every node in dack delete requests
 *  free DACK frame memory (through socket buffer specific API call)
 *  delete entry from queue
 *  free entry struct (virtually)
 * @endcode
 * NOTE: DEBRACATED SYSTEM, unneeded
 */
int dackdel(void *data)
{
    ...
    {       
        ...
        list_for_each_safe(pos, q, &dackfree.list)
        {
            tmp = list_entry(pos, struct vmac_queue, list);
            dev_kfree_skb_any(tmp->frame);
            list_del(pos);
            kfree(tmp);
        }   
    }
...
/**
 * @brief      DACK generation requests queue.
 *
 * @param      data  The data
 * 
 * @code{.unparsed}
 ...
 *  sleep 10-100 usecs
 *  run through queue
 *  call prepDACK for encoding and type(in this function type represents round value for DACK frame)
 *  decreases number of nodes in queue counter
 *  delete entry from queue
 *  free memory. (physical)
 * @endcode
 */
int dackgen(void *data)
{
    ...
    {       
        ...
        //if (dackreqnum != 0)
        {
            list_for_each_safe(pos, q, &dqueue.list)
            {
                tmp = list_entry(pos, struct vmac_queue, list);
                prepDACK(tmp->enc, tmp->seq);
                spin_lock(&dackfreelok);
                dackreqnum--;
                list_del_init(pos);
                spin_unlock(&dackfreelok);
